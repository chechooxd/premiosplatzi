...
class QuestionIndexViewTests(TestCase):
        ...
        def test_two_past_questions(self):
            ...
            self.assertQuerysetEqual(
                response.context['latest_question_list'],
                [past_question_two, past_question_one]
            )

        def test_tow_future_question(self):
            """The questions index page arent't desplayed on the indesx."""
            future_question_one = create_question("Future question 1", days=30)
            future_question_two = create_question("Future question 2", days=40)
            response = self.client.get(reverse('polls:index'))
            self.assertContains(response, "No polls are available.")
            self.assertQuerysetEqual(response.context['latest_question_list'], [])
